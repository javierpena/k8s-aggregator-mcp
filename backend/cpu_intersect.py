...
def get_proc_info(cpu_filter=None):
...
def find_cgroup_mismatches(procs, ignore_cgroups=None, ignore_procs=None, verbose=False):
    ...
    for pid, info in procs.items():
        if cg := info['cgroup']:
            if cg not in ignore_cgroups and info['name'] not in ignore_procs:
...
def print_stats(procs, ignore_cgroups=None, ignore_procs=None):
    ...
    no_cgroup = []
    for pid, info in procs.items():
        if info['name'] in ignore_procs:
            continue
        if cg := info['cgroup']:
            if cg not in ignore_cgroups:
                by_cgroup.setdefault(cg, []).append(pid)
        else:
...
if __name__ == '__main__':
    ...
    procs = get_proc_info(cpu_filter)

    if args.stats:
        print_stats(procs, ignore_cgroups, ignore_procs)
        exit(0)

    mismatches = find_cgroup_mismatches(procs, ignore_cgroups, ignore_procs, args.verbose)
